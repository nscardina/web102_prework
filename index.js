...
const numTotalContributions = GAMES_JSON.reduce((acc, game) => acc + game.backers, 0)

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `${numTotalContributions.toLocaleString("en-US")}`
...
function filterUnfundedOnly() {
    ...
    const gamesNotMetGoal = GAMES_JSON.filter(game => game.pledged < game.goal)
...
const displayStr = `A total of $${
    totalAmountRaised.toLocaleString("en-US")
} has been raised for ${GAMES_JSON.length} ${
    (GAMES_JSON.length === 1) ? "game" : "games"
...
const topPledgeGameElem = document.createElement("p")
topPledgeGameElem.innerHTML = first.name
firstGameContainer.append(topPledgeGameElem)



// do the same for the runner up item
const runnerUpPledgeItem = document.createElement("p")
runnerUpPledgeItem.innerHTML = second.name
secondGameContainer.append(runnerUpPledgeItem)

const search = () => {
    const searchTerm = document.getElementById("search").value
    const gamesMatchingSearch = GAMES_JSON.filter(
        game => game.name.toLowerCase().includes(searchTerm.toLowerCase()) 
        || game.description.toLowerCase().includes(searchTerm.toLowerCase())
    )
    deleteChildElements(gamesContainer)
    addGamesToPage(gamesMatchingSearch)
}
